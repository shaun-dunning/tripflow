...
import AppSessionControls, { initials } from "@/components/AppSessionControls";
    ...
    <>
      ...
      >
        <div className="flex items-center gap-1 rounded-[1.6rem] border border-white/70 bg-white/88 px-1.5 py-1.5 shadow-[0_14px_40px_rgba(15,23,42,0.16)] backdrop-blur-xl">
          ...
          {/* Account avatar — separated by a subtle divider */}
          {user && (
            <>
              <div className="w-px h-6 bg-slate-200 mx-0.5 flex-none" />
              <button
                onClick={() => setAccountOpen(true)}
                aria-label="Account"
                className="flex-none flex items-center justify-center w-9 h-9 rounded-[0.9rem] bg-slate-950 text-white text-[11px] font-black transition-all active:scale-90 active:opacity-80"
              >
                {initials(user)}
              </button>
            </>
