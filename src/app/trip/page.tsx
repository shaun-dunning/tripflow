    ...
    <div className="flex flex-col gap-5 pb-6">
      ...
      <div>
        ...
        <div className="relative h-56 w-full overflow-hidden">
          ...
          {/* Countdown pill */}
          <div className="absolute right-4 bg-white/20 backdrop-blur-md border border-white/30 text-white rounded-2xl px-3 py-2 text-center" style={{ top: "clamp(12px, env(safe-area-inset-top), 44px)" }}>
        ...
        <div className="mx-3 mb-3 mt-1 rounded-2xl overflow-hidden border border-slate-100 bg-white shadow-sm">
          {/* Departure flight */}
          {trip?.startDate && (() => {
            const dep = new Date(trip.startDate + "T12:00:00");
            dep.setDate(dep.getDate() - 1);
            const depLabel = dep.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase();
            return (
              <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-50">
                <div className="flex-none w-16">
                  <p className="text-[10px] font-black uppercase text-slate-400">{depLabel}</p>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-slate-500 font-semibold truncate">
                    Fly to {trip.destination?.split(",")[0] ?? "Maui"}
                  </p>
                </div>
          ...
          {days.map((day, idx) => (
            <div
              key={day.id}
              className={`flex items-center gap-3 px-4 py-3${idx < days.length - 1 ? " border-b border-slate-50" : ""}`}
            >
              <div className="flex-none w-16">
                {day.status === "today" ? (
                  <span className="inline-block rounded-full bg-sky-500 px-2 py-0.5 text-[9px] font-black text-white uppercase tracking-wide">Today</span>
                ) : (
                  <p className={`text-[10px] font-black uppercase ${day.status === "past" ? "text-slate-300" : "text-slate-500"}`}>{day.label}</p>
                )}
              </div>

              <div className="flex-1 min-w-0">
                <p className={`text-xs truncate ${day.status === "past" ? "text-slate-400" : day.status === "today" ? "text-slate-900 font-bold" : "text-slate-700 font-semibold"}`}>
                  {day.theme}
                </p>
              </div>
              ...
              <div className="flex items-center gap-1 flex-none">
                {day.activities.slice(0, 4).map((a, i) => (
                  <span key={i} className={`text-base${day.status === "past" ? " opacity-40" : ""}`}>{a.emoji}</span>
                ))}
                {day.status === "past" && (
          ...
          <div className="border-t border-slate-50 flex items-center gap-3 px-4 py-2.5">
            ...
            {upNextTrip && (
              <div>
                ...
                <button
                  ...
                  className="mt-2 w-full flex items-center justify-center gap-2 border border-dashed border-slate-200 rounded-2xl py-3 text-xs font-semibold text-slate-400 hover:border-slate-300 hover:text-slate-500 transition-colors"
                >
